        ...
        <div class="sticky-sidebar">
            ...
            <div class="widget widget-products">
                <div class="title-link-wrapper mb-2">
                    <h4 class="title title-link font-weight-bold">{{ __('More products') }}</h4>
                </div>
                @foreach ($moreProducts as $moreProduct)
                    <div class="product product-widget">
                        <figure class="product-media">
                            <a href="#">
                                <img src="{{ $moreProduct->imagePreview() }}" alt="{{ $moreProduct->name }}"
                                    width="100" height="113" />
                            </a>
                        </figure>
                        <div class="product-details">
                            <h4 class="product-name">
                                <a href="{{ route('ecommerce.product.show', $moreProduct) }}">{{ $moreProduct->name }}</a>
                            </h4>
                            <div class="ratings-container">
                                <div class="ratings-full">
                                    <span class="ratings" style="width: {{ $moreProduct->getStarsPercentageAVG() }}%;"></span>
                                    <span class="tooltiptext tooltip-top"></span>
                                </div>
                            </div>
                            <div class="product-price">{{ $moreProduct->getPriceToString() }}</div>
                        </div>
                    </div>
                @endforeach
